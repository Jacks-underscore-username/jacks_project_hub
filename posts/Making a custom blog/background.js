...
fetch('../../mem.json')
  ...
  .then(data => {
    ...
    Object.keys(orderedLines).forEach(
      file => (biggestLineCount = Math.max(biggestLineCount, orderedLines[file].length))
    )
  })
  .catch(error => {
...
function typeChar(ctx, canvas, skip) {
  ...
  if (skip || currentLine == lineToWrite || (currentLine.length * fontWidth >= width && Math.random() < 0.25)) {
    if (!skip) {
      if (hasWrittenFileName) {
        currentLine = '|' + currentLine
        writtenLines.unshift([lineNumber + 1, currentLine])
      } else writtenLines.unshift([0, currentLine])
      ...
      if (lineNumber == orderedLines[currentFile].length) {
        ...
        hasWrittenFileName = false
        lineToWrite = wrapInComment(currentFile)
      } else lineToWrite = orderedLines[currentFile][lineNumber]
    }
    ctx.fillStyle = '#000'
    ...
    writtenLines.forEach((line, y) => {
      ...
      if (line[0] - 1 > -1) ctx.fillText(line[0], (lineOffset - String(line[0]).length * fontWidth) / 2, textY)
    ...
    if (hasWrittenFileName) {
      ...
      ctx.fillText(
        lineNumber + 1,
        (lineOffset - String(lineNumber + 1).length * fontWidth) / 2,
        Math.floor(height / fontSize - 1) * fontSize
      )
    ...
    if (hasWrittenFileName)
      ctx.fillText(
        ...
        lineOffset + currentLine.length * fontWidth,
        Math.floor(height / fontSize - 1) * fontSize
      )
    else
      ctx.fillText(
        currentLine.charAt(currentLine.length - 1),
        (currentLine.length - 1) * fontWidth,
        Math.floor(height / fontSize - 1) * fontSize
      )
  }
}
    ...
    if (Date.now() - lastTypeTime >= 1000 / currentSpeed) {
      ...
      if (Math.random() < 0.25)
        currentSpeed = Math.min(
          charsPerSec.max,
          Math.max(charsPerSec.min, currentSpeed + ((Math.random() * 2 - 1) * (charsPerSec.max - charsPerSec.min)) / 4)
        )
      if (charsToWait > 0) {
        charsToWait--
        if (charsToWait % 10 == 0) {
