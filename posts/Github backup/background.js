...
const colors = []
const setColor = ctx => {
  if (colors[colorIndex] === undefined) colors.push({ flickerTime: 0, color: '#0f0' })
  ctx.strokeStyle = ctx.fillStyle = colors[colorIndex].color
  if (colors[colorIndex].flickerTime === 0) ctx.shadowBlur = lineWidth * 2
...
const updateColors = () => {
  for (const color of colors) {
    if (color.flickerTime > 0) {
      color.flickerTime--
      if (color.flickerTime === 0) color.color = '#0f0'
    } else if (Math.random() < flickerChance) {
      color.flickerTime = Math.ceil(Math.random() * 50)
      color.color = `#00${Math.ceil(Math.random() * 12 * 11 + 12)
        .toString(16)
        .padStart(2, '0')}00`
    }
  }
...
const updateOffsets = () => {
  for (const offset of offsets) {
    if (offset.flickerTime > 0) {
      offset.flickerTime--
      if (offset.flickerTime === 0) offset.value = offset.normal
    } else if (Math.random() < flickerChance) {
...
const stars = new Array(100)
  ...
  .map(() => ({
    ...
    y: Math.random(),
    size: Math.random(),
    parts: Math.round(Math.random() * 2) + 2,
...
export function update(canvas, ctx) {
  ...
  flickerChance = 0.1 ** (((Math.cos(Date.now() / 1000) + 1) / 2) * 5)
  ...
  stars.forEach(star => {
    for (let index = 0; index < star.parts; index++) {
      ...
      ctx.ellipse(
        ...
        (size / 50) * star.size,
        (size / 250) * star.size,
        Math.PI * (star.rotation + index / star.parts),
        0,
        Math.PI * 2
      )
      ctx.fill()
      star.x += getOffset(0.0001, 0)
      star.y += getOffset(0.0001, 0)
    }
  })
